...
export const CheckboxContainer = styled.div<{isChecked: boolean}>`
    ...
    align-items: center;
    gap: 0.5rem;
    ${({ isChecked }) => isChecked ? `color: #430E41;` : `color: ${GV('white')};`}
...
export const StyledCheckbox = styled.div<{isChecked: boolean}>`
    ...
    width: 16px;
    height: 16px;
    ${({ isChecked }) => isChecked ? `background-color: #780640;` : `background-color: #232323;`}
    ${({ isChecked }) => isChecked ? `box-shadow: 0px 1px 0px 0px #552493 inset;` : ``}
